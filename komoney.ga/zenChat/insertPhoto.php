...
if(isset($_FILES['file']['name'])){
    ...
    $result=$db->prepare("update users SET pp='{$new_name_json}' where id={$_COOKIE['useridKoMoney']}");
    $result->execute();
    echo $new_name;
    ...
    $new_name_json=json_encode($new_name);
    $result=$db->prepare("update users SET cp='{$new_name_json}' where id={$_COOKIE['useridKoMoney']}");
    $result->execute();
    echo $new_name;
}

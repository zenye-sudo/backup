...
$virtualBalance=$db->prepare("select rac from users where id={$_COOKIE['useridKoMoney']}");
$virtualBalance->execute();
//For Check Balance end
$virtualBalanceFetch=$virtualBalance->fetch();
if($decode->time=="15:35:00") {
    if($totalBetPrice<$virtualBalanceFetch['rac']){
        ...
        if($betTimestampDate<$nowTimestampDate || $betTimestampTime<$nowTimestampTime){
            ...
            if($nowTimestampTime<$betTimestampTime-600){
                foreach ($decode->list as $key => $value) {
                    $insertConfirmVocher = $db->prepare("insert into rac(user_id,num,price,type,status,wl,fromDate,targetDate) VALUES (:user_id,:num,:price,:type,:status,:wl,NOW(),:targetDate)");
                ...
                $nowVirtualBalance=$virtualBalanceFetch['rac']-$totalBetPrice;
                $updateBalance=$db->prepare("UPDATE users set rac='{$nowVirtualBalance}' WHERE id={$_COOKIE['useridKoMoney']}");

...
if($type==0){ //For twod
    if($check->rowCount()==0){
        $close=$db->prepare("update rac set status=0,wl='l',close=1 where id={$id}");
        $close->execute();
        echo "Closed";
    }else{
        if($num==$checkFetch['2d']){
            $update=$db->prepare("UPDATE rac set status=0,wl='w',close=0 WHERE id={$id}");
            $update->execute();
            ...
            $update=$db->prepare("UPDATE rac set status=0,wl='l',close=0 WHERE id={$id}");
            $update->execute();
            echo "Lose";
        }
    }
    ...
    if($check->rowCount()==0){
        ...
        if($num==$checkFetch['2d']){
            ...
            $update=$db->prepare("UPDATE rac set status=0,wl='t',close=0 WHERE id={$id}");
            $update->execute();
            $pricePrize=$price*100;
            $gettingBalance=$db->prepare("select rac from users where id={$_COOKIE['useridKoMoney']} and username='{$_COOKIE['usernameKoMoney']}'");
            $gettingBalance->execute();

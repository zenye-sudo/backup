...
$activeF=$db->prepare("select users.id,users.username,users.pp from users inner join login_details on login_details.uid=users.id where login_details.last_activity>DATE_SUB(NOW(),INTERVAL 5 SECOND) AND users.id!={$_COOKIE['userid']}");
...
foreach ($result1->fetchAll() as $item){
    ...
    if($data['id']!=$uid2){
        $pp=json_decode($data['pp']);
        $array1[]=["id1"=>$item['id'],"id"=>$data['id'],"username"=>$data['username'],"pp"=>$pp,"countt"=>$friendCount->fetchColumn()];
    }
    }
}
